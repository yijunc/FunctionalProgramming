  ...
  assert (E: n + m + (n + p) = (n + n) + (m + p)).
   { rewrite plus_assoc.  rewrite plus_assoc. 
     assert (H1: n+m+n=n+n+m).
      { rewrite <- plus_assoc. 
        rewrite <- plus_assoc. 
        assert (eq: m+n=n+m). { apply plus_comm. }
        rewrite eq. reflexivity.  
      }
     rewrite H1. reflexivity.
   }
  rewrite E in H. apply ev_ev__ev with (n:= n+n).
  apply H.
...
Example test_total1 : total_relation 7 113.
Proof. apply tr. Qed.
...
Inductive empty_relation : nat -> nat -> Prop :=
  | emp : forall n m, False -> empty_relation n m.

Example test_er1 : ~ empty_relation 2 7.
Proof. intro H. inversion H. apply H0. Qed. 
...
Proof.
  (* FILL IN HERE *) 
  intros n m E. induction n as [| n' IHn'].
  - apply O_le_n.
  - inversion E as [| m' E'].
    + apply le_n.
...
Proof.
  ...
  apply leb_correct. apply leb_complete in Enm. apply leb_complete in Emo.
  apply le_trans with m. apply Enm. apply Emo.
Qed.
...
(* FILL IN HERE *)
  ...
  - intro E. induction E.
    ...
    + rewrite <- IHE. unfold fR. rewrite <- plus_n_Sm. reflexivity.
    + unfold fR in IHE. rewrite <- plus_n_Sm in IHE. inversion IHE.
      unfold fR. reflexivity. 
    + unfold fR. unfold fR in IHE. rewrite <- IHE. apply plus_comm.
  - generalize dependent o. induction m, n. 
    + intros. inversion H. simpl. apply c1.
    + intros o E. simpl in E. inversion E. apply c3. apply R_1.
    + intros o E. unfold fR in E. rewrite plus_comm in E. simpl in E.
      rewrite <- E. apply c2. apply IHm. unfold fR. rewrite plus_comm. reflexivity.
    + intros o E. unfold fR in E. simpl in E. rewrite <- E. apply c2. 
      apply IHm. unfold fR. reflexivity.
Qed.
(** [] *)
...
Theorem app_nil_r : forall {X : Type} (l : list X),
  l ++ [] = l.
Proof. intros X l. induction l as [ | h t IHt].
 - reflexivity.
...
Fixpoint In {T} (x : T) (l : list T) : Prop :=
  match l with
  | [] => False
  | a :: l' => x = a \/ In x l'
  end.

Example test_In1 : In 5 [2;5].
Proof. simpl. right. left. reflexivity. Qed.

Example test_In2 : ~ In 3 [2;5].
Proof. intro H. simpl in H. destruct H as [H1 | H2].
 - inversion H1.
...
Proof.
  ...
  - simpl. apply MStarApp. 
    ...
    + apply IHt. intros t' H1. apply H. simpl. right; apply H1.
Qed.
(** [] *)
...
Proof.
 ...
 intros P [ ] H.
  - inversion H. split.
    ...
    + intro; apply H0.
  - inversion H. split.
    + intro H1; apply H0 in H1. inversion H1.
...
Lemma plus_eq0 : forall m n, m + n = 0 -> 
    m = 0 /\ n = 0.
Proof. intros m n H. destruct m as [| m'].
...
Proof.
  (* FILL IN HERE *) 
 intros n l H. induction l as [| h t IHt].
  - simpl. intro H1; inversion H1.
  - simpl in H. apply plus_eq0 in H. destruct H as [H1 H2].
    apply IHt in H2. destruct (beq_natP n h) as [H3 | H3].
    + inversion H1.
    + intro H4. simpl in H4. destruct H4 as [H4 | H4].
...
Inductive nostutter {X:Type} : list X -> Prop :=
 (* FILL IN HERE *)
  ...
  | ns_cons : forall x x' l, x <> x' /\ nostutter (x' :: l) 
              -> nostutter (x :: x' :: l)
.
...
(* FILL IN HERE *) 
  Proof. repeat (apply ns_cons; split; try apply beq_nat_false_iff; auto ) .
  apply ns_one.
  Qed.
...
(* FILL IN HERE *) 
  Proof. intro H.
  inversion H. destruct H1 as [ _ H1].
  inversion H1. destruct H5 as [H51 H52]. apply H51. reflexivity.
Qed.
