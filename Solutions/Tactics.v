...
Proof.
  (* FILL IN HERE *) 
  intros X x y z l j _ e2. inversion e2. reflexivity. Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *) 
  intros X x y z l j eq. inversion eq. 
Qed.
(** [] *)
...
Proof.
  ...
  - intros m eq. destruct m as [| m'].
    + reflexivity. 
    + rewrite <- plus_n_Sm in eq. simpl in eq. inversion eq.
  - intros m eq. destruct m as [| m'].
    + simpl in eq. inversion eq.
    + rewrite <- plus_n_Sm in eq. rewrite <- plus_n_Sm in eq. 
      simpl in eq. inversion eq as [eq1]. apply IHn' in eq1.
...
Proof.
  ...
  - intros n eq. destruct n as [| n'].
    + inversion eq.
    + simpl. apply IHt. inversion eq. reflexivity.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *) 
  intros X l1. induction l1 as [| h t IHt].
  - intros l2 x n eq. simpl. simpl in eq. apply eq. 
  - intros l2 x n eq. simpl. simpl in eq. destruct n as [| n'].
    + inversion eq.
    + apply f_equal. apply IHt with x. inversion eq. reflexivity.
Qed.
(** [] *)
...
Lemma app_length_hd : forall (X: Type) (h : X) (l1 l2 : list X),
    length (l1 ++ h :: l2) = S (length (l1 ++ l2)).
Proof. intros X h l1 l2. induction l1 as [| a t IHt].
  - reflexivity.
...
Proof.
  ...
  - simpl. intros n eq. rewrite app_length_hd.
    destruct n as [| n'].
    + inversion eq.
    + simpl. rewrite <- plus_n_Sm. apply f_equal. apply f_equal.
...
Proof.
  (* FILL IN HERE *) 
  intros P P1 P2 P3 P4. induction m as [| m' IHm'].
...
Proof.
  ...
  - intros l1 l2 e. simpl in e. inversion e. reflexivity. 
  - intros l1 l2 e.  simpl in e. destruct (split t) as [t1 t2].
    inversion e. simpl. apply f_equal. 
...
Proof.
  (* FILL IN HERE *) 
  intros f b. destruct (f true) eqn : eq1.
  - destruct b.
    + rewrite eq1. rewrite eq1. apply eq1.
    + destruct (f false) eqn : eq2.
      { rewrite eq1. apply eq1. }
      { rewrite eq2. apply eq2. }
  - destruct b.
    + rewrite eq1. destruct (f false) eqn : eq2.
      { apply eq1. }
      { apply eq2. }
    + destruct (f false) eqn : eq2.
...
Proof.
  (* FILL IN HERE *) 
SearchAbout beq_nat.
  intros n m. destruct (beq_nat n m) eqn : Hnm.
  - apply beq_nat_true in Hnm. rewrite Hnm. apply beq_nat_refl.
  - destruct (beq_nat m n) eqn : Hmn.
    + apply beq_nat_true in Hmn. rewrite Hmn in Hnm.
      rewrite <- beq_nat_refl in Hnm. symmetry. apply Hnm.
    + reflexivity.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *) 
  intros n m p eq1 eq2. apply beq_nat_true in eq1. apply beq_nat_true in eq2.
...
(* FILL IN HERE *) 
  ...
  - intros lt1 lt2 H1 H2. destruct lt1 as [| l1h l1t].
    ...
    + destruct lt2 as [| l2h l2t].
      * inversion H1.
      * destruct h as [h1 h2].
        inversion H2. rewrite H4. simpl. 
        assert (eq: split t = (l1t, l2t)).
        { apply IHt. simpl in H1. inversion H1. reflexivity. apply H4. }
...
Example existsb'_test1 : existsb' (beq_nat 5) [0;2;3;6] = false.
Proof. reflexivity. Qed.

Example existsb'_test2 : existsb' (andb true) [true;true;false] = true.
Proof. reflexivity. Qed.

Example existsb'_test3 : existsb' oddb [1;0;0;0;0;3] = true.
Proof. reflexivity. Qed.

Example existsb'_test4 : existsb' evenb [] = false.
Proof. reflexivity. Qed.

Theorem existsb_existsb' : forall (X : Type) (test : X -> bool) (l : list X),
  existsb test l = existsb' test l.
Proof. intros X test l. induction l as [| h t IHht].
  - reflexivity. 
  - unfold existsb'. simpl. destruct (test h) eqn : TF.
    + reflexivity.
    + simpl. unfold existsb' in IHht. apply IHht.
Qed.
