     ...
     foo'_ind :
        forall (X : Type) (P : foo' X -> Prop),
              (forall (l : list X) (f : foo' X),
                    __P f_____________________ ->
                    __P (C1 X l f)____________   ) ->
             _______P (C2 X )___________________________ ->
             forall f : foo' X, ___P f_____________________
...
(* FILL IN HERE *)
Definition plus_assoc_f ( n m p : nat) :=
  n + (m + p) = (n + m) + p.

Theorem plus_assoc'' : forall n m p : nat,
  plus_assoc_f n m p.
Proof. induction n as [| n' IHn'].
  - intros. unfold plus_assoc_f. simpl. reflexivity.
  - intros. unfold plus_assoc_f. unfold plus_assoc_f in IHn'.
    simpl. rewrite IHn'. reflexivity.
Qed.

Definition plus_comm_f (n m : nat) :=
  n + m = m + n.

Theorem plus_comm''' : forall n m : nat,
  plus_comm_f n m.
Proof. induction n as [|n' IHn'].
  - intro m. unfold plus_comm_f. rewrite <- plus_n_O. reflexivity.
  - intro m. unfold plus_comm_f. unfold plus_comm_f in IHn'.
