...
Example square_test1 : square 10 = 100.
Proof. reflexivity. Qed.

Theorem plus_comm_assoc : forall n m p : nat,
  n + (m + p) = m + (n + p).
...
Theorem mult_distri_l : forall n m p : nat,
  n * (m + p) = n * m + n * p.
Proof. intros n m p. induction n as [| n' IHn'].
  ...
  - simpl. rewrite IHn'. rewrite <- plus_assoc.
    ...
    { rewrite plus_comm_assoc. reflexivity. }
    rewrite H1. reflexivity.
Qed.
...
Theorem mult_comm : forall m n : nat,
  m * n = n * m.
Proof. intros m n. induction m as [| m' IHm'].
  - simpl. rewrite mult_0_r. reflexivity.
  - simpl. assert (H: S m' = 1 + m'). { reflexivity. }
    rewrite H. rewrite mult_distri_l. 
    rewrite mult_1_r. rewrite IHm'. reflexivity.
Qed.

(*
Theorem sum_square : forall m n : nat, 
  square (m + n) = square m + square n + 2 * m * n.
Proof. intros m n. induction m as [| m' IHm'].
  - simpl. rewrite <- plus_n_O. reflexivity.
  - rewrite plus_Sn_m. simpl.
    rewrite <- plus_n_O. rewrite <- plus_n_O.
    rewrite IHm'. 
    assert (H : square m' + 2 * m' * n + square n + (m' + n + (m' + n)) + 1 =
square m' + (m' + m') + 1 + (n + (m' + S m') * n) + square n)
*)
(* ################################################################# *)
...
Theorem mult_plus_distr_r' : forall n m p : nat,
  (n + m) * p = (n * p) + (m * p).
Proof. intros n m p. rewrite mult_comm.
   rewrite mult_distri_l.
   rewrite mult_comm.
   assert (H2: p * m = m * p).
